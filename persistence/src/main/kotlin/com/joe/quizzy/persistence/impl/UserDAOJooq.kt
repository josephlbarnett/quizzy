...
open class UserDAOJooq
    ...
    constructor(
        ...
        private fun getRecord(
            dsl: DSLContext,
            id: UUID,
        ): UsersRecord? = dsl.selectFrom(Tables.USERS).where(Tables.USERS.ID.eq(id)).fetchOne()

        @Timed
        override fun get(id: UUID): User? = getRecord(ctx, id)?.into(User::class.java)
        ...
        override fun getByEmail(email: String): User? =
            ctx.selectFrom(Tables.USERS).where(Tables.USERS.EMAIL.eq(email)).fetchOneInto(User::class.java)
        ...
        override fun savePassword(
            ...
            ctx.transactionResult { config ->
                config
                    .dsl()
                    .update(Tables.USERS)
                    .set(Tables.USERS.AUTH_CRYPT, cryptedPass)
                    ...
                    if (thingId == null) {
                        ...
                        if (existing != null) {
                            ...
                            existing.from(
                                ...
                                *existing
                                    .fields()
                                    .filter { it != Tables.USERS.AUTH_CRYPT && it != Tables.USERS.ADMIN }
                                    .toTypedArray(),
                            )
                            existing
                        } else {
        ...
        override fun create(
            ...
            ctx.transactionResult { config ->
                val instanceId =
                    config
                        .dsl()
                        .select(USER_INVITE.INSTANCE_ID)
                        .from(USER_INVITE)
